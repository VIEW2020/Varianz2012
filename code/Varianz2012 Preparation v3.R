...
# Sabastiano / Suneela VARIANZ 2012 Population v3

# Updated May, 2020
# Created by Billy Wu

# v3 update
# - Utilises a widened VARIANZ definition 
...
# Import
VARIANZ <- read.fst("source_data/R/VARIANZ/EXTENDED_VARIANZ_2012.fst",
                    as.data.table = T)

VARIANZ <- BOTHTABLE[VARIANZ, nomatch = 0] # Starting 4582807

# 1.  Age at 2012-12-31
# Filter age 30-74
COHORT <- VARIANZ[EN_AGE >= 30 & EN_AGE <=74]  # -2170506 / 2412301


# 2. Meshblock
EN_MESHBLOCK <- read.fst("source_data/R/LOOKUP TABLES/MB NZDep/PHO_EN_MESHBLOCK_2004_2019.fst",
                         as.data.table = T)

# Capture last known meshblock information nearest Q4.2012
# Allow for a 1 year lookforward (as per relaxed-VARIANZ of PHO)
EN_MESHBLOCK <- EN_MESHBLOCK[VSIMPLE_INDEX_MASTER %in% VARIANZ$VSIMPLE_INDEX_MASTER]

EN_MESHBLOCK <- EN_MESHBLOCK[, meshblock := MB_2012_Q4]

EN_MESHBLOCK[is.na(meshblock), 
             meshblock := apply(.SD, 1, function(i){
               vals <- zoo::na.locf(i, fromLast = T)
               return(vals[1]) }), 
             .SDcols = c("MB_2013_Q1", "MB_2013_Q2", "MB_2013_Q3", "MB_2013_Q4")]

COHORT[, EN_MESHBLOCK := EN_MESHBLOCK$meshblock[match(VSIMPLE_INDEX_MASTER, EN_MESHBLOCK$VSIMPLE_INDEX_MASTER)]]
  
COHORT <- COHORT[!is.na(COHORT$EN_MESHBLOCK)] # -39375 / 2372926

# 3.  Dispensing 28 days after death
# nb - Ensure no record of dispensing of any drug > 28 days after date of death 
...
ALL_PHARMS <- rbindlist(lapply(files.list, 
                               function(x){
                                 ...
                                 return(DAT[VSIMPLE_INDEX_MASTER %in% COHORT$VSIMPLE_INDEX_MASTER])
                               }))

write.fst(ALL_PHARMS, "V:/sbar288/working/ALL_PHARMS_2013_2018_v3.fst")

ALL_PHARMS[, dod := COHORT$EN_DOD[match(VSIMPLE_INDEX_MASTER, COHORT$VSIMPLE_INDEX_MASTER)]]
...
COHORT[, phh_post_dod := DOD_PHARMS$exclude[match(VSIMPLE_INDEX_MASTER, DOD_PHARMS$VSIMPLE_INDEX_MASTER)]]

COHORT <- COHORT[phh_post_dod==0 | is.na(phh_post_dod)][, -"phh_post_dod", with=F] # removes 1006, 2371860 remaining

# 4. Make sure each individual is still alive on 2012-12-31
# i.e remove those who died before day 1 of follow-up
COHORT <- COHORT[is.na(EN_DOD) | EN_DOD > "2012-12-31"] # -13779 / 2358081

# Meshblock data
MBLOOKUP <- readRDS("V:/common_lookups/MB NZDep/MB0613_NZDep0613_Concordance.rds")

# NZDep
COHORT[, en_nzdep_d := MBLOOKUP$NZDep2006[match(EN_MESHBLOCK, MBLOOKUP$MB_2006)]]
COHORT[is.na(en_nzdep_d), 
       en_nzdep_d := MBLOOKUP$NZDep2013[match(EN_MESHBLOCK, MBLOOKUP$MB_2013)]]
COHORT[, en_nzdep_q := ifelse(en_nzdep_d %in% c(1, 3, 5, 7, 9),
                              (en_nzdep_d + 1) / 2,
                              en_nzdep_d / 2)]

write.fst(COHORT, "sbar288/working/Baseline_EndPart_Av3.fst", compress = 75)
...
# Baseline <- read.fst("sbar288/working/Baseline_EndPart_Av3.fst", as.data.table = T)
...
# Save
write.fst(ALL_COHORT_EVENTS, "sbar288/working/ALL_COHORT_EVENTS_v3.fst", 75)
write.fst(ALL_COHORT_DIAGS, "sbar288/working/ALL_COHORT_DIAGS_v3.fst", 75)

ALL_EVENTS <- read.fst("sbar288/working/ALL_COHORT_EVENTS_v3.fst", as.data.table=T)
ALL_DIAGS <- read.fst("sbar288/working/ALL_COHORT_DIAGS_v3.fst", as.data.table=T)
...
files.list <- list.files("source_data/R/VDR/", pattern="VDR")[1:8]

for(file in files.list){
  
  VDR <- read.fst(paste0("source_data/R/VDR/", file), as.data.table = T)
...
# save
write.fst(Baseline, "sbar288/working/Baseline_EndPart_Bv3.fst", 75)
...
# Exclude history of CVD: 
Baseline <- Baseline[hx_broad_cvd==0] # - 130143 / 2227938

# Save
write.fst(Baseline, "sbar288/working/Baseline_EndPart_Cv3.fst", compress = 75)
...
write.fst(PHARMS, "sbar288/working/ALL_PHARMS_2008_2012_v3.fst")
...
# Exclude history of loopdiuretics (5 year 3x)
Baseline <- Baseline[ph_loopdiuretics_prior_5yrs_3evts==0] # -19168 / 2208770

write.fst(Baseline, "sbar288/working/Baseline_EndPart_Dv3.fst", 75)


# ---- E.   End of followup ----
# Nb: This should adjust for last contact date for people without 

# Remove people with unknown gender "U”
# Gender - remove U; code M=1, F=0
Baseline <- Baseline[GENDER!="U"] #remove 91 people / 2208679 remain
Baseline[, gender_code := +(GENDER=="M")]

# Remove people without a valid NZDep score
Baseline <- Baseline[!is.na(en_nzdep_q)]#  - 6984  / 2201695  remain

# End of FU event: For people with an event (90945)
Baseline$end_fu_date_event <- as.Date(apply(Baseline[, c("EN_DOD", "out_broad_cvd_adm_date"), with = F], 1, function(x){
...
summary(Baseline$end_fu_date_event)

# End of Study: For people without an event (2110750)
for(year in 2013:2017){
 
 VARIANZ <- read.fst(paste0("V:/source_data/R/VARIANZ/EXTENDED_VARIANZ_", year, ".fst"),
                     as.data.table = F)
 
 date.var <- paste0("contact_", year)
 
 Baseline[, (date.var) := VARIANZ$LAST_CONTACT_DATE[match(VSIMPLE_INDEX_MASTER, VARIANZ$VSIMPLE_INDEX_MASTER)]]
 
 print(paste0(year, " completed"))
 
}

date.vars <- c("contact_2013", "contact_2014", "contact_2015", "contact_2016", "contact_2017")

Baseline[, last_contact_date := as.Date(apply(.SD, 1, function(i)
 if(all(is.na(i))){
  NA
 } else {
  max(i, na.rm = T)
...
Baseline[, (date.vars) := NULL]

# End of Followup
Baseline[, end_fu_date := ifelse(!is.na(end_fu_date_event), end_fu_date_event, last_contact_date)]

# Pho contact in 2012 flag
...
write.fst(Baseline, "sbar288/working/Baseline_EndPart_Ev3.fst", 75)


# ---- F.  Final Tidyup ----- 

names(Baseline)[-1] <- tolower(names(Baseline)[-1])

Baseline_FINAL <- Baseline %>%
  select(VSIMPLE_INDEX_MASTER, gender_code, en_age, en_prtsd_eth, en_sole_ethgp, en_solecombo_eth, en_nzdep_q, en_dod, last_contact_date, end_fu_date,
...
# Keep naming consistent with v1 and v2
names(Baseline_FINAL)[3] <- "nhi_age"

# Name changes:
# Loopdiur, antidiabetes, en_prtsd_eth
fst::write.fst(Baseline_FINAL, "sbar288/data/R_VARIANZ_2012_v3.fst", compress = 75)
haven::write_dta(Baseline_FINAL, "sbar288/data/Stata14_VARIANZ_2012_v3.dta", version = 14)
haven::write_dta(Baseline_FINAL, "smeh005/2012 VARIANZ/Stata14_VARIANZ_2012_v3.dta", version = 14)
feather::write_feather(Baseline_FINAL, "sbar288/data/Py_VARIANZ_2012_v3.feather")
feather::write_feather(Baseline_FINAL, "D:/sbar288/Varianz2012/data/Py_VARIANZ_2012_v3.feather")
