...
def main():
    ...
    # Trained models
    models = listdir(hp.data_dir + 'iter4/log_' + hp.gender + '/')
    ...
    for i in range(len(models)):
        ...
        net.load_state_dict(torch.load(hp.data_dir + 'iter4/log_' + hp.gender + '/' + models[i], map_location=hp.device))
        ...
        for i in tqdm(range(num_embeddings)):
            with torch.no_grad():
                x_b = torch.zeros((1, x.shape[1]), device=hp.device)
                codes_b = torch.zeros((1, 1), device=hp.device)
                month_b = torch.zeros((1, 1), device=hp.device)
                diagt_b = torch.zeros((1, 1), device=hp.device)
                codes_b[0] = (i+1)
